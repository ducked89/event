...
if($this->isConnected()){
		...
		switch ($params) {
							...
							if($limit[0]->nb<5){
								...
								if($saved){
									...
									$saved = $Agent->ajouterContent(array(
										...
										'content'		=>'Vous avez une nouvel évenement.',
									...
									$this->redirigerVers($this->direct.'events/index');
								}
								else{
			...
			case 'view':
			{
				if(isset($_GET['idu']) && preg_match("/^[0-9]+$/i", $_GET['idu']))
				{
					...
					if(count($tempEvaluations)>0 && isset($tempEvaluations[0]->id))
					{
						...
						$this->chargerViewLayout($this->layout, $this->direct.'events/view',$this->datas);
					}
					else $this->redirigerVers("administrator/events/index");
				}
				else $this->redirigerVers("administrator/events/index");
			}
			break;
			...
			case 'overview':
			{
				if(isset($_GET['idu']) && preg_match("/^[0-9]+$/i", $_GET['idu']))
				{
					...
					if(count($tempEvaluations)>0)
					{
						...
						$this->chargerViewLayout($this->layout, $this->direct.'events/overview',$this->datas);
					}
					else $this->redirigerVers("administrator/events/overview/?idu=".$idu."&nf=yes");
				}
				else $this->redirigerVers("administrator/events/");
			}
			break;
			...
			case 'summarize':
			{
				if(isset($_GET['idu']) && preg_match("/^[0-9]+$/i", $_GET['idu']))
				{
					...
					if(count($tempEvaluations)>0)
					{
						...
						//die();
						$this->datas['evaluations'] = $tempEvaluations2;
						$this->chargerViewLayout($this->layout, $this->direct.'events/summary',$this->datas);
					}
					else $this->redirigerVers("administrator/events/summarize/?idu=".$idu."&nf=yes");
				}
				else $this->redirigerVers("administrator/events/");
			}
			break;
			...
			case 'final':
			{
				if(isset($_GET['idu']) && preg_match("/^[0-9]+$/i", $_GET['idu']))
				{
					...
					if(count($tempEvaluations)>0)
					{
						...
						//die();
						$this->datas['evaluations'] = $tempEvaluations2;
						$this->chargerViewLayout($this->layout, $this->direct.'events/final',$this->datas);
					}
					else $this->redirigerVers("administrator/events/");
				}
				else $this->redirigerVers("administrator/events/");
			...
			case 'print':
			{
				if(isset($_GET['idu']) && preg_match("/^[0-9]+$/i", $_GET['idu']))
				{
					...
					if(count($tempEvaluations)>0)
					{
						...
						//die();
						$this->datas['evaluations'] = $tempEvaluations2;
						$this->chargerViewLayout("layout_print", $this->direct.'events/print',$this->datas);
					}
					else $this->redirigerVers("administrator/events/summarize/?idu=".$idu.'&fd=no');
				}
				else $this->redirigerVers("administrator/events/");
			}
			break;
			...
			case 'printall':
			{
				if(isset($_GET['idu']) && preg_match("/^[0-9]+$/i", $_GET['idu']))
				{
					...
					if(count($tempEvaluations)>0)
					{
						...
						//die();
						$this->datas['evaluations'] = $tempEvaluations2;
						$this->chargerViewLayout("layout_print_commentaire", $this->direct.'events/print',$this->datas);
					}
					else $this->redirigerVers("administrator/events/");
				}
				else $this->redirigerVers("administrator/events/");
				...
				if(isset($_GET['idu']) && preg_match("/^[0-9]+$/i", $_GET['idu']))
				{
					...
					if(isset($_POST["saveEvaluation"])){
						...
						if(!empty($title))
						{
							...
							if(isset($check[0]) && isset($check[0]->id))
							{
								...
								if($saved){
									...
									$this->datas['mdatas'] = $_POST;
									$this->datas['error']['notsaved'] =true;
									$this->chargerViewLayout($this->layout, $this->direct.'events/edit',$this->datas);
								}
							}
							else{
								$this->datas['mdatas'] = $_POST;
						...
						if(isset($found[0]->id))
						{
							...
							$this->redirigerVers("administrator/events/");
						}
					}
				}
				else{
			...
			case 'activate':
			{
				if(isset($_POST['opt']) && $_POST['opt']==1)
				{
					...
					$this->chargerViewLayout($this->layout, $this->direct.'events/activeallevent', $this->datas);
				}
			}
			break;
			...
			case 'desactivate':
			{
				if(isset($_POST['opt']) && $_POST['opt']==0)
				{
					...
					$this->chargerViewLayout($this->layout, $this->direct.'events/desactiveallevent', $this->datas);
				}
			}
			break;
			...
			case 'grid':
			{
				...
				$this->chargerViewLayout($this->layout, $this->direct.'events/grid', $this->datas);
			}
			break;
	...
	else
	{
		...
		$this->chargerViewLayout($this->layout, $this->direct.'events/index', $this->datas);
	}
}
