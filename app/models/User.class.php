...
class User extends Model
{
	...
	function __construct(){
		...
		*/
			...
			$req = $PDO->prepare("SELECT users.id, users.login, users.roleid, users.firstlogin, employes.lastname, employes.firstname, employes.photo, employes.sex , employes.phone, employes.email, employes.position, employes.adresse, employes.extension FROM users LEFT JOIN employes ON users.id = employes.iduser 
				...
				try{
					...
					if(count($data)>0){
						...
						$this->resetAttempts($da["connectname"]);
						return true;
					}
					else{ 
						$this->addAttempts($da["connectname"]);
						return false; 
					}
				}
		...
		function addAttempts($uname){
			global $PDO;
			$req = $PDO->prepare("UPDATE  users SET numberattempts=numberattempts+1, lastattempts=NOW()
				WHERE login='".$uname."' LIMIT 1" );
			try{
				$req->execute();
				return true;
			}
		...
		function resetAttempts($uname){
			global $PDO;
			$req = $PDO->prepare("UPDATE  users SET numberattempts=0
				WHERE login='".$uname."' LIMIT 1" );
			try{
				$req->execute();
				return true;
		...
		function checkAttempts($uname){
			global $PDO;
			$req = $PDO->prepare("SELECT numberattempts FROM  users WHERE login='".$uname."' LIMIT 1" );
			try{
				$req->execute();
				$data = $req->fetchAll();
				if(isset($data[0]->numberattempts))
					return $data[0]->numberattempts;
				else return 0;
			}
			catch (PDOException $e){
				return 0;
		...
		function checkLastAttempts($uname){
			global $PDO;
			$req = $PDO->prepare("SELECT DATE_FORMAT(TIMEDIFF(NOW(), lastattempts), '%i')  FROM  users WHERE login=".$uname." LIMIT 1" );
			//SELECT DATE_FORMAT(TIMEDIFF(NOW(), lastattempts), '%i') AS DIFF FROM `users` 
			try{
				$req->execute();
				$data = $req->fetchAll();
				return $data[0]->numberattempts;
